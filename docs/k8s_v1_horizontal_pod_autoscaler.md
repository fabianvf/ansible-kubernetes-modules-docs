...
#### Synopsis
Manage the lifecycle of a horizontal_pod_autoscaler object. Supports check mode, and attempts to to be idempotent.
...
| spec_max_replicas  |  max_replicas  |   |  | |  upper limit for the number of pods that can be set by the autoscaler; cannot be smaller than MinReplicas.  |
| spec_min_replicas  |  min_replicas  |   |  | |  lower limit for the number of pods that can be set by the autoscaler, default 1.  |
| spec_scale_target_ref_api_version  |  scale_target_ref_api_version  |   |  | |  API version of the referent  |
| spec_scale_target_ref_kind  |  scale_target_ref_kind  |   |  | |  Kind of the referent;  |
| spec_scale_target_ref_name  |  scale_target_ref_name  |   |  | |  Name of the referent;  |
| spec_target_cpu_utilization_percentage  |  target_cpu_utilization_percentage  |   |  | |  target average CPU utilization (represented as a percentage of requested CPU) over all the pods; if not specified the default autoscaling policy will be used.  |
