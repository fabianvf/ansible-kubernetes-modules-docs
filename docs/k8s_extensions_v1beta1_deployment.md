...
| spec_paused  |   |  | |  Indicates that the deployment is paused and will not be processed by the deployment controller.  |
...
| spec_revision_history_limit  |   |  | |  The number of old ReplicaSets to retain to allow rollback. This is a pointer to distinguish between explicit zero and not specified.  |
...
| spec_rollback_to_revision  |   |  | |  The revision to rollback to. If set to 0, rollback to the last revision.  |
| password  |   |  | |  Provide a password for connecting to the API. Use in conjunction with I(username).  |
| spec_strategy_type  |   |  | |  Type of deployment. Can be "Recreate" or "RollingUpdate". Default is RollingUpdate.  |
...
| spec_replicas  |   |  | |  Number of desired pods. This is a pointer to distinguish between explicit zero and not specified. Defaults to 1.  |
