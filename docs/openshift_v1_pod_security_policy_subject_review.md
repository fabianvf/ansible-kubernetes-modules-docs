...
## openshift_v1_pod_security_policy_subject_review
...
| spec_groups  |     |    | <ul></ul> |  groups is the groups you're testing for.  |
...
| spec_user  |     |    | <ul></ul> |  user is the user you're testing for. If you specify "user" but not "group", then is it interpreted as "What if user were not a member of any groups. If user and groups are empty, then the check is performed using *only* the serviceAccountName in the template.  |
