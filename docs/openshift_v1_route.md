...
#### Synopsis
Manage the lifecycle of a route object. Supports check mode, and attempts to to be idempotent.
...
| spec_alternate_backends  |  alternate_backends  |   |  | |  alternateBackends allows up to 3 additional backends to be assigned to the route. Only the Service kind is allowed, and it will be defaulted to Service. Use the weight field in RouteTargetReference object to specify relative preference.  |
| spec_host  |  host  |   |  | |  host is an alias/DNS that points to the service. Optional. If not specified a route name will typically be automatically chosen. Must follow DNS952 subdomain conventions.  |
| spec_path  |  path  |   |  | |  Path that the router watches for, to route traffic for to the service. Optional  |
| spec_port_target_port  |  port_target_port  |   |  | |  The target port on pods selected by the service this route points to. If this is a string, it will be looked up as a named port in the target endpoints port list. Required  |
| spec_tls_ca_certificate  |  tls_ca_certificate  |   |  | |  caCertificate provides the cert authority certificate contents  |
| spec_tls_certificate  |  tls_certificate  |   |  | |  certificate provides certificate contents  |
| spec_tls_destination_ca_certificate  |  tls_destination_ca_certificate  |   |  | |  destinationCACertificate provides the contents of the ca certificate of the final destination. When using reencrypt termination this file should be provided in order to have routers use it for health checks on the secure connection. If this field is not specified, the router may provide its own destination CA and perform hostname validation using the short service name (service.namespace.svc), which allows infrastructure generated certificates to automatically verify.  |
| spec_tls_insecure_edge_termination_policy  |  tls_insecure_edge_termination_policy  |   |  | |  insecureEdgeTerminationPolicy indicates the desired behavior for insecure connections to a route. While each router may make its own decisions on which ports to expose, this is normally port 80. * Allow - traffic is sent to the server on the insecure port (default) * Disable - no traffic is allowed on the insecure port. * Redirect - clients are redirected to the secure port.  |
| spec_tls_key  |  tls_key  |   |  | |  key provides key file contents  |
| spec_tls_termination  |  tls_termination  |   |  | |  termination indicates termination type.  |
| spec_to_kind  |  to_kind  |   |  | |  The kind of target that the route is referring to. Currently, only 'Service' is allowed  |
| spec_to_name  |  to_name  |   |  | |  name of the service/target that is being referred to. e.g. name of the service  |
| spec_to_weight  |  to_weight  |   |  | |  weight as an integer between 0 and 256, default 1, that specifies the target's relative weight against other target reference objects. 0 suppresses requests to this backend.  |
| spec_wildcard_policy  |  wildcard_policy  |   |  | |  Wildcard policy if any for the route. Currently only 'Subdomain' or 'None' is allowed.  |
