...
## openshift_v1_o_auth_access_token
...
#### Synopsis
Manage the lifecycle of a o_auth_access_token object. Supports check mode, and attempts to to be idempotent.
...
| user_uid  |     |    | <ul></ul> |  UserUID is the unique UID associated with this token  |
...
| refresh_token  |     |    | <ul></ul> |  RefreshToken is the value by which this token can be renewed. Can be blank.  |
...
| authorize_token  |     |    | <ul></ul> |  AuthorizeToken contains the token that authorized this token  |
