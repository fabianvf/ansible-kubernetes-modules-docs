...
| spec_external_id  |  external_id  |   |  | |  External ID of the node assigned by some machine database (e.g. a cloud provider). Deprecated.  |
| spec_pod_cidr  |  pod_cidr  |   |  | |  PodCIDR represents the pod IP range assigned to the node.  |
| spec_provider_id  |  provider_id  |   |  | |  ID of the node assigned by the cloud provider in the format: <ProviderName>://<ProviderSpecificNodeID>  |
| spec_taints  |  taints  |   |  | |  If specified, the node's taints.  |
| spec_unschedulable  |  unschedulable  |   |  | |  Unschedulable controls node schedulability of new pods. By default, node is schedulable.  |
