...
| spec_provider_id  |     |    | <ul></ul> |  ID of the node assigned by the cloud provider in the format: <ProviderName>://<ProviderSpecificNodeID>  |

| spec_external_id  |     |    | <ul></ul> |  External ID of the node assigned by some machine database (e.g. a cloud provider). Deprecated.  |
...
| spec_pod_cidr  |     |    | <ul></ul> |  PodCIDR represents the pod IP range assigned to the node.  |
...
| spec_unschedulable  |     |    | <ul></ul> |  Unschedulable controls node schedulability of new pods. By default, node is schedulable.  |
...
| spec_taints  |     |    | <ul></ul> |  If specified, the node's taints.  |
