...
#### Synopsis
Manage the lifecycle of a project object. Supports check mode, and attempts to to be idempotent.
...
| description  |     |    | <ul></ul> |  Provides a brief overview or narrative for the project.  |
...
| spec_finalizers  |     |    | <ul></ul> |  Finalizers is an opaque list of values that must be empty to permanently remove object from storage  |
...
| display_name  |     |    | <ul></ul> |  Provides a descriptive name for the project.  |
