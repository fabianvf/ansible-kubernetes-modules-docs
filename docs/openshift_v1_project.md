...
| description  |  |   |  | |  Provides a brief overview or narrative for the project.  |
| display_name  |  |   |  | |  Provides a descriptive name for the project.  |
...
| spec_finalizers  |  finalizers  |   |  | |  Finalizers is an opaque list of values that must be empty to permanently remove object from storage  |
