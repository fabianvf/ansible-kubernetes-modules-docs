...
#### Synopsis
Manage the lifecycle of a host_subnet object. Supports check mode, and attempts to to be idempotent.
...
| egress_i_ps  |     |    | <ul></ul> |  EgressIPs is the list of automatic egress IP addresses currently hosted by this node  |

| host_ip  |     |    | <ul></ul> |  HostIP is the IP address to be used as a VTEP by other nodes in the overlay network  |
...
| subnet  |     |    | <ul></ul> |  Subnet is the CIDR range of the overlay network assigned to the node for its pods  |
...
| host  |     |    | <ul></ul> |  Host is the name of the node. (This is the same as the object's name, but both fields must be set.)  |
...
host_subnet:
  ...
  contains:
    ...
    egress_i_ps:
      description:
      - EgressIPs is the list of automatic egress IP addresses currently hosted by
        this node
      type: list
      contains: str
    host:
      description:
      - Host is the name of the node. (This is the same as the object's name, but
