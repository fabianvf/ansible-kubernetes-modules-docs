...
#### Synopsis
Manage the lifecycle of a resource_quota object. Supports check mode, and attempts to to be idempotent.
...
| spec_hard  |  hard  |   |  | |  Hard is the set of desired hard limits for each named resource.  |
| spec_scopes  |  scopes  |   |  | |  A collection of filters that must match each object tracked by a quota. If not specified, the quota matches all objects.  |
