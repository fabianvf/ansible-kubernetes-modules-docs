...
| spec_scopes  |     |    | <ul></ul> |  A collection of filters that must match each object tracked by a quota. If not specified, the quota matches all objects.  |
...
| spec_hard  |     |    | <ul></ul> |  Hard is the set of desired hard limits for each named resource.  |
