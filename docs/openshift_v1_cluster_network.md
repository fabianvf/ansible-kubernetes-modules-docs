...
| cluster_networks  |  |   |  | |  ClusterNetworks is a list of ClusterNetwork objects that defines the global overlay network's L3 space by specifying a set of CIDR and netmasks that the SDN can allocate addressed from.  |
...
| plugin_name  |  |   |  | |  PluginName is the name of the network plugin being used  |
...
| service_network  |  |   |  | |  ServiceNetwork is the CIDR range that Service IP addresses are allocated from  |
