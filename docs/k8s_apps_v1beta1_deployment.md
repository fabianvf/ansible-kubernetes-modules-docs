...
| spec_paused  |  paused  |   |  | |  Indicates that the deployment is paused.  |
...
| spec_revision_history_limit  |  revision_history_limit  |   |  | |  The number of old ReplicaSets to retain to allow rollback. This is a pointer to distinguish between explicit zero and not specified. Defaults to 2.  |
